...
export function joinDescriptors(
  ...
  function clone_with_abstract_value(d: Descriptor) {
    ...
    if (!IsDataDescriptor(realm, d)) {
      invariant(d.get !== undefined && d.set !== undefined);
      dc.joinCondition = joinCondition;
      dc.get = (getAbstractValue(d.get, realm.intrinsics.empty): any);
      dc.set = (getAbstractValue(d.set, realm.intrinsics.empty): any);
      return dc;
    }
  ...
  if (d1 === undefined) {
    ...
    if (!d3.writable || !d3.configurable || !d3.enumerable || !!d3.set || !!d3.get) {
